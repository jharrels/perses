...
func FromArgs(args []string) *Invalidator {
	inv := &Invalidator{}
	fl := flag.NewFlagSet("perses", flag.ExitOnError)
	fl.StringVar(&inv.dist, "dist", "", "CloudFront distribution ID")
	fl.StringVar(&inv.query, "query", "", "CloudFront invalidationID")
	...
	fl.DurationVar(&inv.wait, "wait", 0*time.Minute, "Time out for waiting on invalidation to complete. Set to 0 to exit without waiting.")
	fl.Usage = func() {
...
// Query queries the underlying distribution. Does not return a value.
func (inv *Invalidator) Query(id string) (err error) {
...
func (inv *Invalidator) Done(id string) (done bool, err error) {
	...
	fmt.Println(*result.Invalidation.Status)
